...
# Funkcja 'assign_flow' przypisuje wartość przepływu 'a' dla każdej krawędzi wykresu na podstawie macierzy 'matrix'. Używa funkcji 'shortest_path'
# z biblioteki NetworkX, aby znaleźć najkrótszą ścieżkę między każdą parą wierzchołków grafu.
...
# Funkcja 'assign_capacity' przypisuje wartość pojemności 'c' do każdej krawędzi wykresu na podstawie wartości przepływu 'a' i współczynników.
...
# Funkcja T oblicza średnią wydajność sieci grafowej 'graph' uwzględniając macierz 'matrix', współczynnik m oraz maksymalną wydajność T_max.
...
# Funkcja 'reliability' oblicza niezawodność sieci grafowej, biorąc pod uwagę macierz "matrix", maksymalną wydajność T_max,
# p, współczynnik m i inne parametry. Wykonuje kilka próbnych iteracji,
# w każdej z których krawędzie są losowo usuwane z prawdopodobieństwem p i obliczana jest niezawodność sieci.
...
# Funkcje 'fast_assign_flow', 'test1', 'test2' i 'test3' wykonują różne testy w sieci grafu.
# Generują wykresy oraz obliczają niezawodność sieci po zmianach.
...
# Funkcja 'main' tworzy wykres, generuje macierz N, wywołuje funkcje w celu przypisania przepływu
# i pojemności oraz wizualizuje wykres za pomocą biblioteki Matplotlib.
def main():
